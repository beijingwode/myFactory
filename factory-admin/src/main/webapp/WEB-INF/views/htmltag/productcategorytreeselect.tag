...
@var modelId = modelId!'pid'; //隐藏要提交的id (实体属性)
...
@var rootNodeName = rootNodeName!"菜单列表"; //虚拟顶级节点名称
@var chkboxType = chkboxType!""; //1不选择父节点
@var scenario = scenario!"";//树形结构使用的场景（默认商品分类。规格管理、属性管理  等等）
@if(isLayer == "true"){
<div class="clearfix ${class} input-group" style="${style}">
	...
	<span class="input-group-btn">
		...
		data-mode="page" data-url="${ctxPath}/tag/productcategorytree"
		data-data="{scenario:'${scenario}',url:'${url}',id:'${id}',nameId:'${nameId}',pIdKey:'${pIdKey}',selectIds:'${selectIds}',curId:'${curId}',checked:'${checked}',rootNodeName:'${rootNodeName}',isLayer:'${isLayer}'}" 
...
@}else{
	...
	<script>
		$.ajax({
			url:"${ctxPath}/tag/productcategorytree",
			type:"post",
			data:{url:'${url}',id:'${id}',nameId:'${nameId}',pIdKey:'${pIdKey}',
				selectIds:'${selectIds}',curId:'${curId}',checked:'${checked}',isLayer:"false",
				treeSelectId:'${treeSelectId}',rootNodeName:'${rootNodeName}',chkboxType:'${chkboxType}',test:'test'},
