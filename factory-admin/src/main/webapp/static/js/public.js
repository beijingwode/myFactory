				...
				$.ajax({
        			...
        			if(data.success) {
        				if(params.success == undefined){
        					layer.msg('<span class="red bigger-120">处理成功</span>', 1, 1,function(){
        						if(params.reloadurl){
            						location.reload();
            					}else{
        				...
        				layer.msg('<span class="red bigger-120">'+data.msg+'</span>', 2, 8);
        			
        			}else {
        				layer.alert('<span class="red bigger-120">处理失败！</span>', 8, !1);
        			}
        		}).fail(function(error){
        			layer.msg('<span class="red bigger-120">系统异常</span>', 2, 8);
        		});
			},params.title);
			return false;
		} else	if(mode == 'del' || mode == 'delete'){
			var loadi;
			layer.confirm(params.msg,function(index){
				$.ajax({
					url:params.url,
        			...
        			if(data>0||data.success) {
        				...
        				layer.msg('<span class="red bigger-120">删除失败:'+data.msg+'</span>', 2, 8);
...
$(function(){
	...
	$(document).on('click','[data-mode]',function(){
		...
		//data-mode 为mergeReceipt所对应的data-data的值是经常改变的。
		if("mergeReceipt"==data.mode){
			
			var data1 = $("#mergeReceipt").attr("data-data"); //detail
			if(data1==""){
				return ;
			}else{
