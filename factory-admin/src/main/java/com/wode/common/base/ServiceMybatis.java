...
import com.wode.common.mybatis.mapper.BaseMapper;
import com.wode.common.spring.utils.SpringContextHolder;
import com.wode.common.util.Collections3;
import com.wode.common.util.StringConvert;
...
public abstract class ServiceMybatis<T> implements BaseService<T>{
	...
	public List<T> select(T record) {
		if(record instanceof BaseEntity){
			((BaseEntity)record).set("delFlag", Constant.DEL_FLAG_NORMAL);
		}
	...
	public int insert(T record) {
		if(record instanceof BaseEntity){
			...
			entity.set("delFlag", Constant.DEL_FLAG_NORMAL);
			entity.set("createBy", SysUserUtils.getLoginUser().getId()+","+
					SysUserUtils.getLoginUser().getName());
	...
	public int updateByPrimaryKey(T record) {
		if(record instanceof BaseEntity){
			BaseEntity entity=(BaseEntity)record;
			SysUser sysUser = SysUserUtils.getLoginUser();
			if(sysUser != null){
				entity.set("updateBy",sysUser.getId()+","+
						SysUserUtils.getLoginUser().getName());
			}
			entity.set("updateDate", new Date());
		}
	...
	public int updateByPrimaryKeySelective(T record) {
		if(record instanceof BaseEntity){
			BaseEntity entity=(BaseEntity)record;
			entity.set("updateBy",SysUserUtils.getLoginUser().getId()+","+
					SysUserUtils.getLoginUser().getName());
			entity.set("updateDate", new Date());
		}
