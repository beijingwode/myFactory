...
public class EntBenefitFacadeImpl implements EntBenefitFacade{
	...
	public int autoSeason(Long entId, int year, int season, String updUser) {
		...
		if(rtn==null || rtn.size() == 0) {
			...
			ist.setCurrentTransferSum(BigDecimal.ZERO);	//本季度划入内购券额度
	...
	public int grantBenefit(Long apprId, Long entId, BigDecimal limit,
		...
		entBenefitApprService.updateSelect(appr);
		////////////////////////////////////////////////////////////////////////////////////////
	...
	public int dealEntBenefit(Long entId, String opCode, EntSeasonAct esa,
		...
		if(signed.compareTo(BigDecimal.ZERO) > 0) {
			if("116".equals(opCode)) {
				...
				//内购券余额=本季度获得内购券+本季度划入内购券额度+上季度残
	...
	public int paySaleBill(Long saleBillId, Long userId, String updName, Long flowId) {
		...
		saleBill.setLastUpdateTime4(new Date());
		saleBill.setPayTime(new Date());
		saleBillService.update(saleBill);
		////////////////////////////////////////////////////////////////////////////////////////
		
		//账目处理
		int rtn= this.dealEntBenefit(saleBill.getSupplierId(), "120", esa, saleBill.getPayPrice(), BigDecimal.ZERO, saleBillId, flowId, "系统账单结算，货款入账。结算单号："+saleBill.getBillId(), updName,false);
		
		//先款商家自动提现
		if(NumberUtil.isGreaterZero(saleBill.getPayPrice()) && saleBill.getCloseType()>=4) {
			// 货款结算,且货款>0
			SupplierDuration sd = sds.getById(saleBill.getSupplierId());
			if(sds.isPayFirst(sd)) {
				SupplierTransfer appr = new SupplierTransfer();
	        	appr.setSupplierId(saleBill.getSupplierId());
	        	appr.setFinanceCode(sd.getFinanceCode());
	        	appr.setStatus(1);
	        	appr.setAmount(saleBill.getPayPrice());
	        	appr.setCreateDate(new Date());
	        	appr.setCreateUser(userId);
	        	appr.setRejectNote("先款商家自动提现");
	...
	public int cashTransferFail(Long transferId, String note, String updName, Long flowId) {
		...
		if(3 != transfer.getStatus()) return -2; //该记录已审批过
		////////////////////////////////////////////////////////////////////////////////////////
		...
		//账目处理
		int rnt= this.dealEntBenefit(transfer.getSupplierId(), "124", esa, transfer.getAmount(), BigDecimal.ZERO, transferId, flowId, "提现转账失败："+note, updName);
