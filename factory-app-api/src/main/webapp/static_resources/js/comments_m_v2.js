...
function ajaxGetComments(){
	$.ajax({
		url :'product/comments.json?productId='+productId+'&page=1&pageSize=30',
		type : "GET",
				...
				if(list.length>0){
					...
					for(var i=0;i<list.length ;i++) {
						...
						if(list[i].shopLink && list[i].shopLink!="" && list[i].shopLink!="null") {
							shopLink="/shop/page?shopId=" + list[i].shopLink;
						}
			    	...
			    	$(".comment_box").hide();
			    	$(".estimate_box").html('<p class="p_box">商品暂无评价</p>');
			    	$(".estimate_box .p_box").show();	
				}			
			}
		},
		error : function() {}
