		...
		success: function(data){
			if(data.success){
				...
				for(var i=0;i<hits.length;i++) {
					...
					html +='</dt>';
					html +='<dd class="dd1"><a href="javascript:go2Search(\''+productId+'&specificationsId='+specificationsId+'\')">'+name+'</a></dd>';
					html +='<dd><span>￥'+(rprice.toFixed(2))+'+'+parseFloat(hits[i].maxFucoin).toFixed(2)+'券</span></dd>';
					html +='<dd class="dd3"><span>电商价:￥'+(hits[i].price.toFixed(2))+'</span></dd>';
					html +='</dl>';
					html +='</li>';
				}
				$("#more ul").append(html);
