...
$(document).ready(function() {
	...
	if(typeof(btnName)!=undefined && btnName!='' && btnName=='match'){//匹配
		$("title").html("欲领清单");
		$(".bottom_box a").attr("class","exchange");
		$(".bottom_box a").text("我想领");
		$(".TogetherToBuy_help").show();//如何换领
		$(".selfDelivery_btm span").text("公司统一收货（免运费） ");
	}
	...
	if(productIds!="" && productIds && productIds!=null){
		...
		for (var i = 0; i < productIdArr.length; i++) {
			...
			if(pageStock && pageStock!=null){
				productPageStock += pageStock+",";
			}
		}
	}

	var pageKey =sessionStorage.getItem("pageKey");
	if(pageKey) {
	} else {
		pageKey="";
	}
	$.ajax({
		url : jsBasePath +'order/confirmCart.user?partNumbers='+partNumbers+'&uid='+uid+"&productPageStock="+productPageStock+"&pageKey="+pageKey,
		type : "GET",
		dataType: "json",  //返回json格式的数据  
	    async: false,
	    cache:false,
			...
			if (data.success) {
				...
				for (var k = 0; k < cart.length; k++) {
				    ...
				    for(var i=0;i<cartItems.length;i++) {
					    ...
					    html +='<dd class="dd3"><p>￥<span>'+((cartItems[i].realPrice)).toFixed(2)+'</span>+'+(cartItems[i].maxCompanyTicket).toFixed(2)+'内购券</p><em>X<i>'+cartItems[i].quantity+'</i><em></dd>';
				...
				//内购券
			    /*if (result.companyTicket>totalfuli) {
			    	isMaxCompanyTicket =true;
				}else{
					isMaxCompanyTicket = false;
				}*/
			    var userLimitTicketVoList =result.userLimitTicketVoList;//我的优惠券
			    var cashCoupon =0;//现金优惠券
			    var ticketCoupon =0;//内购优惠券
			    var couponId = "";//优惠券id
			    if(userLimitTicketVoList && userLimitTicketVoList!=null && userLimitTicketVoList.length>0){
			    	for (var i = 0; i < userLimitTicketVoList.length; i++) {
			    		var cashBalance =userLimitTicketVoList[i].cashBalance;
			    		var ticketBalance =userLimitTicketVoList[i].ticketBalance;
			    		if(cashBalance==null){
			    			cashBalance= 0;
			    		}
			    		...
			    		if(userLimitTicketVoList[i])
			    		var userLimitTicketId = userLimitTicketVoList[i].id;
			    		if(userLimitTicketId){
			    			couponId+=userLimitTicketId+"_"+cashBalance+"_"+ticketBalance+",";
			    		}
			    		cashCoupon+=cashBalance;
			    		ticketCoupon+=ticketBalance;
					}
			    ...
			    if(ticketCoupon>totalfuli){//内购券优惠大于购买商品总内购券
			    	ticketCoupon = totalfuli;
			    	totalfuli =ticketCoupon-totalfuli;
			    }else{
			    	totalfuli = totalfuli-ticketCoupon;
			    }
			    if(cashCoupon>totalRealPrice){//现金券优惠大于购买商品总价
			    	cashCoupon = totalRealPrice;
			    	totalRealPrice =cashCoupon-totalRealPrice;
			    	//$("#xianjin").hide();
			    }else{
			    	totalRealPrice = totalRealPrice-cashCoupon;
			    }
			    $("#coupon_ticket").val(ticketCoupon);
			    $("#coupon_cash").val(cashCoupon);
			    if(cashCoupon>0 && ticketCoupon>0){
			    	$("#coupon").html("优惠券：￥"+cashCoupon.toFixed(2)+"+"+ticketCoupon.toFixed(2)+"内购券");
			    }else if(cashCoupon>0 && ticketCoupon<=0){
			    	$("#coupon").html("优惠券：￥"+cashCoupon.toFixed(2));
			    }else if(cashCoupon<=0 && ticketCoupon>=0){
			    	$("#coupon").html("优惠券：￥"+ticketCoupon.toFixed(2)+"内购券");
			    }else {
			    	$("#coupon").hide();
			    }
			    ...
			    if (result.companyTicket>totalfuli) {//用户内购券大于商品所需 默认选中
				    $("#fuli_sel").val(1);
		    	...
		    	if(result.isSelfDelivery){
					toggleSel("selfDelivery");
				}
				calculateTotalFreight();
				////计算运费 (20171101 内购会 临时默认选中自提)
...
function cartCreate(){
	...
	var data='';
	if(btnName=='match'){//匹配
		url=jsBasePath +'exchangeOrder/create.user?fromType=weixin&uid='+uid
		data= {"userId":uid,"name":$("#name").val(),"mobile":$("#mobile").val(),"address":$("#address").val(),"message":message,
				...
				"useCash":useCash,"shippingAddressId":$("#shippingAddressId").val(),"sku_nums":sku_nums,"sku_freights":sku_freights,"selfDelivery":$("#selfDelivery_sel").val(),"freeSwap":$("#freeSwap_sel").val()};
	}else{
		url=jsBasePath +'order/create.user?fromType=weixin&uid='+uid
		data={"userId":uid,"name":$("#name").val(),"mobile":$("#mobile").val(),"address":$("#address").val(),"message":message,
				"useCompanyTicket":useCompanyTicket,"useExchangeTicket":useExchangeTicket,"useExchangeCash":useExchangeCash,"useExchangeSelf":useExchangeSelf,
				"useCash":useCash,"shippingAddressId":$("#shippingAddressId").val(),"sku_nums":sku_nums,"sku_freights":sku_freights,"selfDelivery":$("#selfDelivery_sel").val(),"limitTickets":$("#coupon_id").val()};
	}
	if(url=='' || uid=='') return;
	$.ajax({
		url : url,
			...
			if (data.success) {
				...
				if(result.status == 1) {
					...
					if(btnName=='match'){//匹配
						...
						window.location = jsBasePath+'pay/pay_success.user?uid='+uid+'&orderId='+result.orderId+'&suborderId='+'&fromWay='+orderType+'&type=0';
					}
				} else {
					...
					if(btnName=='match'){//匹配
						...
						window.location=jsBasePath +'pay/page.user?uid='+uid+'&orderId='+result.orderId+'&totalFee='+(result.realPrice-useCash)+'&orderType='+orderType+'&pruductId='+"&backNum="+backNum+'&type=0';
					}
				}
			} else {
				var msg = data.msg;				
			...
			if (data.success) {
			    ...
			    if (blnExp) {
					...
					//if(isMaxCompanyTicket){
				    	...
				    	if(btnName && btnName!=null && btnName!='' && btnName=='match'){
				    		$(".bottom_box a").attr("class","exchange");
				    	}
				    	$(".bottom_box a").attr("href","javascript:orderSubmit();")
					//}
