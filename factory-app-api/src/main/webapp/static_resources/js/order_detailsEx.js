...
function ajaxOrderDeatilsData(){
	if(uid=="") return;
	$.ajax({
		url : jsBasePath+'order/detailEx.user?uid='+uid+'&subOrderId='+subOrderId,
		type : "GET",
...
function canOrConfirmOrder(subOrderId,type){
	$.ajax({
		url : jsBasePath+'managerOrderRecord/addManagerOrderRecord.user?uid='+uid+'&subOrderId='+subOrderId+"&operationStatus="+type+"&flag="+2,
		type : "GET",
		dataType: "json",  //返回json格式的数据  
		async: false,
	    cache:false,
