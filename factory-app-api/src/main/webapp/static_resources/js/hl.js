...
function getTsMore(page) {
	$.ajax({
		...
		async: false,
		cache: false,
		url: "pSearch?saleKbn=2&page="+page,
		success: function(data){
			if(data.success){
				...
				for(var i=0;i<hits.length;i++) {
					...
					html +='<li>';
					html +='<dl>';
					html +='<dt><a href="javascript:go2Search(\''+productId+'&pageKey=huanling'+'\')"><img src="'+hits[i].image+'" /></a>';
						/*if(hits[i].saleKbn==1) {
					...
					html +='</dt>';
					html +='<dd class="dd1"><a href="javascript:go2Search(\''+productId+'\')">'+name+'</a></dd>';
