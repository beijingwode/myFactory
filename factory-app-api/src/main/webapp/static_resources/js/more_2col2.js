...
function getTsMore(page) {
	...
	if(moreKey!="" && moreVal!="" ) {
		searchKey = 'saleKbn=5&'+moreKey +"="+moreVal + "&";
	}
	$.ajax({
		type: "get",
		...
		success: function(data){
			if(data.success){
				...
				for(var i=0;i<hits.length;i++) {
					...
					html +='<li>';
					html +='<dl>';
					html +='<dt><a href="javascript:go2Search(\''+productId+'&specificationsId='+specificationsId+'&pageKey='+$("#pageKey").val()+'\')"><img src="'+hits[i].image+'" /></a>';
						/*if(hits[i].saleKbn==1) {
					...
					html +='</dt>';
					html +='<dd class="dd1"><a href="javascript:go2Search(\''+productId+'&specificationsId='+specificationsId+'&pageKey='+$("#pageKey").val()+'\')">'+name+'</a></dd>';
					html +='<dd class="dd3">￥'+(rprice.toFixed(2))+'+'+parseFloat(hits[i].maxFucoin).toFixed(2)+'券</dd>';
					html +='<dd class="dd4"><p>限量'+hits[i].stock+'份</p><a href="javascript:go2Search(\''+productId+'&specificationsId='+specificationsId+'&pageKey='+$("#pageKey").val()+'\')">立即试用</a></dd>';
					html +='</dl>';
					html +='</li>';
				}
				$("#more ul").append(html);
