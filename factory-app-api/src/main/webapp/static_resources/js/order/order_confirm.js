...
$(document).ready(function() {
	...
	// 选券返回
	var limitTicketIds =sessionStorage.getItem("limitTicketIds");
	if(limitTicketIds) {
		$("#coupon_ids").val(limitTicketIds);
	}
	init();
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data.success) {
				...
				// 余额
			    if(result.companyTicket && result.companyTicket>0) {
				...
				for (var k = 0; k < cart.length; k++) {
				    ...
				    for(var i=0;i<cartItems.length;i++) {
					    ...
					    html +='<input type="hidden" name="skuCash" value="'+prvProductRealPrice+'">';
			    ...
			    html +='<input type="hidden" id="allSkuCash" value="'+totalRealPrice.toFixed(2)+'">'
			    html +='<input type="hidden" id="allSkuTicket" value="'+totalfuli.toFixed(2)+'">'
			    $("#orderPrdouct").html(html);
		    	if(result.isSelfDelivery){
					toggleSel("selfDelivery");
				}
				calculateTotalFreight();
				getWithOutTicketBuySkuId();
				calculateLimitTicket($("#coupon_ids").val());
			} else {
...
function cartCreate(){
	...
	data={"userId":uid,"name":$("#name").val(),"mobile":$("#mobile").val(),"address":$("#address").val(),"message":message,
			...
			"useCash":useCash,"shippingAddressId":$("#shippingAddressId").val(),"sku_nums":sku_nums,"sku_freights":sku_freights,"selfDelivery":$("#selfDelivery_sel").val(),"limitTickets":$("#coupon_key").val()};
...
function refreshCash(){
	// 计算福利券
	var allSkuTicket = parseFloat($("#allSkuTicket").val());
	var couponTicket = parseFloat($("#coupon_ticket").val());
	var totalfuli = allSkuTicket-couponTicket;
	var userfuli = parseFloat($("#fuli_user").val());
	
    $("#fuli_need").val(totalfuli);
	if(totalfuli>0) {
	    $("#fuli").html("内购券：￥"+totalfuli.toFixed(2)+"<span></span>");
	    if (userfuli>totalfuli) {//用户内购券大于商品所需 默认选中
		    $("#fuli_sel").val(1);
			...
			$(".popover3 .theme_popover_con2").html('<p class="p1">内购券不足<em>￥'+totalfuli.toFixed(2)+'</em></p><p>您剩余内购券：￥'+userfuli.toFixed(2)+'</p><p>是否前往APP找好友凑券</p>');
