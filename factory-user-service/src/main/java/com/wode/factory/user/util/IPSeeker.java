...
public class IPSeeker {
	 //纯真IP数据库名   
    private String IP_FILE="qqwry.tmp";   
    //保存的文件夹   
    ...
    public static synchronized IPSeeker CreateInstance(String fileName,String dir) {
    	if(instance == null) {
    		instance=new IPSeeker(fileName,dir);
    	}
        ...
        for(long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {   
            ...
            if(temp != -1) {   
                ...
                if(ipLoc.getCountry().indexOf(s) != -1 || ipLoc.getArea().indexOf(s) != -1) {   
                    ...
                    // 得到起始IP   
                    readIP(offset - 4, b4);   
                    entry.beginIp = IpSeekUtil.getIpStringFromBytes(b4);   
                    // 得到结束IP   
                    readIP(temp, b4);   
                    entry.endIp = IpSeekUtil.getIpStringFromBytes(b4);   
                    // 添加该记录   
                    ret.add(entry);   
                }   
            }   
