     ...
     * 更新结果
     */
    function onchangeResult(obj) {
        var $resu = $(obj).parents(".sale_content").find("[name='suborder_result']");
        var value = $(obj).parents(".sale_content").find("[name='subOrderId']").val() + "|_|" + $(obj).parents(".sale_content").find("[name='expressNo']").val();
        $resu.val(value);
    }
    ...
    function radilClick(obj) {
        ...
        $("#" + typenamealert).text(sendAddressHidd);
    ...
    function validatorForm() {
		...
		var $selfDeliverylist = $("[name='selfDelivery']");
        var ret = 0;
        
        var needEx = false;
        for(var i=0;i<$expressNolist.length;i++) {
        	if("1" != $($selfDeliverylist[i]).val()) {
        		needEx = true;
        		if (expressType !="14660000000000001") {
    ...
    function express_change(){
        ...
        }else if(extId == "14660000000000001"){
        	$(".s_name").each(function(i){
        		if(i>0) {
        			$(this).html("货运号：");
        		}
        ...
        if (extId == "14660000000000001") {//厂家直送
    		$(".input_s").each(function(i){
        		$(this).attr("placeholder","此项可不填");
        	});
		}else{
			$(".input_s").each(function(i){
