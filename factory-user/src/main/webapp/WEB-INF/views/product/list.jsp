...
<body>
	...
	<div id="content">
		<div class="position">
		  <c:choose>
			<c:when test="${empty path[0].url}">
			  <a href="/channel${path[0].id}.html">${path[0].name}</a>
			</c:when>
			<c:otherwise>
				<a href="/${path[0].url}">${path[0].name}</a>
			</c:otherwise>
		  </c:choose>
		 	<em>></em><a href="#">${path[1].name}</a><em>></em><a href="#">${path[2].name}</a>
		</div>
		<div class="detail_wrap">
			...
			<div class="left">
				<div class="product_sort">
					<ul>
					  <c:forEach items="${cats.keySet()}" var="cat">
						<li class="lt ${cat.id == path[1].id ? 'after' : ''}">
							<a href="javascript:">${cat.name}</a>
							<ul class="downlist" ${cat.id == path[1].id ? 'style="display:block;"' : ''}>
							  <c:forEach items="${cats.get(cat)}" var="child">
								<li>
									<a href="${basePath}product/list?cat=${path[0].id},${cat.id},${child.id}" ${child.id == path[2].id ? 'style="color:red;"' : ''}>${child.name}</a>
								</li>
							  </c:forEach>
							</ul>
						</li>
					...
					<div class="historyproduct">
						<ul>
						  <c:forEach items="${recommend}" var="rep">
							<li>
								<div class="p-img"><a href="/${rep.id }.html" target="_blank"><img src="${rep.image }" width="174" height="77" alt="${rep.fullName }"></a></div>
								<div class="p-price"><strong>¥${rep.showPrice }</strong></div>
								<div class="p-name">
									<p><a href="/${rep.id }.html" target="_blank">${rep.fullName }</a></p>
								</div>
							</li>
						  </c:forEach>
						</ul>
					...
					<div class="recommend_list">
						<ul>
						  <c:forEach items="${hotProducts}" var="pro" begin="0" end="3">
							<li>
								<div class="l_img">
									<a href="/${pro.id}.html" target="_blank"><img src="${pro.image}" width="104" height="71" alt="${pro.fullName}"></a>
								</div>
								<div class="l_r">
									<p class="p3" style="height: 40px; overflow: hidden;">
										<a href="/${pro.id}.html" target="_blank" title="${pro.fullName}">${pro.fullName}</a>
									</p>
									${pro.showPrice}
							...
							<li class="at">
							  <c:choose>
								<c:when test="${hit.saleKbn == 1}">
								  <div class="picon">
									  <img src="../images/picon1.png" />
								  </div>
							    </c:when>
								<c:when test="${hit.saleKbn == 2}">
								  <div class="picon">
									  <img src="../images/picon_c1.png" />
								  </div>
							    </c:when>
							    <c:when test="${hit.saleKbn == 4}">
								  <div class="picon">
									  <img src="../images/picon_z1.png" />
								  </div>
							    </c:when>
								<c:when test="${hit.saleKbn == 5}">
								  <div class="picon">
									  <img src="../images/picon_t1.png" />
								  </div>
							    </c:when>
						  	  </c:choose>
						  		...
						  		<div class="all_product_pho">
								    ...
								    <a href="${basePath}<fmt:formatNumber value="${hit.productId }" pattern="#0"/>.html?skuId=<fmt:formatNumber value="${hit.minSkuId}" pattern="#0"/>&pageKey=search" target="_blank"><img src="${hit.image}" alt=">${hit.name }"></a>
								</div>
								<h2>
								...
								<p class="p1">
									<span>
										内购价：¥<fmt:formatNumber value="${hit.salePrice }" type="number" groupingUsed="false" maxFractionDigits="2" minFractionDigits="0" />
										<i>+<fmt:formatNumber value="${fucoin}" type="number" groupingUsed="false" maxFractionDigits="2" minFractionDigits="0" />券</i>
									</span>
									<em><fmt:formatNumber value="${hit.discount}" type="number" groupingUsed="false" maxFractionDigits="1" minFractionDigits="0" />折</em>
								</p>
								<p class="p2">
									电商价：￥<fmt:formatNumber value="${hit.price}" type="number" groupingUsed="false" maxFractionDigits="2" minFractionDigits="0" />
								</p> 
								<em><a href="/shop/<fmt:formatNumber value="${hit.shopId }" pattern="#0"/>" target="_blank">${hit.shopName }</a></em>
...
<script type="text/javascript">
	...
	<!--footer end-->
	<script type="text/javascript"
		src="${basePath }resources/js/top_ewm.js"></script>
</body>
</html>
