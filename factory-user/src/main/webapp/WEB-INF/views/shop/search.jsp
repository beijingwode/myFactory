	...
	<div class="Brand_con">
		<div class="B_c_left">
			<div class="product_sort">
				<ul>
					<li style="background: #F8F8F8;"><a href="${basePath}shop/${ss.id}">全部商品</a></li>
				  <c:forEach var="category" items="${categories}" varStatus="categoryStatus">
					<c:choose>
					  <c:when test="${parent != null}">
						<li class="lt ${category.id eq parent ? 'after' : ''}">
							<a href="javascript:">${category.name}</a>
							<ul class="downlist" ${category.id eq parent || category.isExpanding ? 'style="display: block;"' : ''}>
					  </c:when>
					  <c:otherwise>
						<li class="lt ${categoryStatus.index == 0 ? 'after' : ''}">
							<a href="javascript:">${category.name}</a>
							<ul class="downlist" ${categoryStatus.index == 0 || category.isExpanding ? 'style="display: block;"' : ''}>
					  </c:otherwise>
					</c:choose>
			...
			<ul class="B_cr_list">
			  <c:forEach items="${result.hits }" var="hit">
				<li>
					...
					<p class="B_c_img">
						<a href="${basePath}<fmt:formatNumber value="${hit.productId }" pattern="#0"/>.html?skuId=<fmt:formatNumber value="${hit.minSkuId }" pattern="#0"/>&pageKey=shop"  target="_blank"><img src="${hit.image}" width="168px" height="130px"></a>
					</p>
					<h2>${hit.name}</h2>
					<p class="p1">
	...
	<div class="login_popup" style="display: none;">
		<div class="L_popupbox">
			<div class="L_title">
				<p>我的网</p>
				<div class="close_btn">
					<img src="../../images/close.gif" width="14" height="14" alt="close">
				</div>
			</div>
			<div class="L_cont">
				<div id="pError">
